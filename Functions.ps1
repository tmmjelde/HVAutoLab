function rebuild ($vm) {
    ...
    if ($Process){
        $Process = $Process | Where-Object {$_.MainWindowTitle -eq "$vm on $($env:computername) - Virtual Machine Connection"}
    }
    if ($process){
        Write-Verbose -Message "Attempting to stop existing VM Connections"
...
function New-LabVM {
  ...
  if ($ClientConfig.MemoryStartupGB){$MemoryStartupGB = $ClientConfig.MemoryStartupGB}
  if ($ClientConfig.ProcessorCount){$ProcessorCount = $ClientConfig.ProcessorCount}
  if ($OSDiskSizeGB.OSDiskSizeGB){$OSDiskSizeGB = $ClientConfig.OSDiskSizeGB}
  try {
    $VHDPath = "$((Get-VMHost).VirtualHardDiskPath)\$Name"
    if (test-path "$UnattendFolder\AutoUnattend.iso"){remove-item "$UnattendFolder\AutoUnattend.iso"}
    Write-Host "Deleted old Unattend ISO File"
    get-childitem $UnattendFolder -Exclude *.iso | New-ISOFile -path "$UnattendFolder\AutoUnattend.iso"
    "Created new Unattend ISO File"
    New-VM -Name $VMName -MemoryStartupBytes ($($MemoryStartupGB)*1GB) -Generation 1
    "Created new VM"
    Set-VM $Name -ProcessorCount $ProcessorCount
    "Set processor count"
    Set-VM $Name -AutomaticCheckpointsEnabled $AutomaticCheckpointsEnabled
    "Set automatic checkpoint"
  

    New-VHD -Path $VHDPath\$Name.vhdx -SizeBytes ($($OSDiskSizeGB)*1GB) -Dynamic
    "Created VHD"
    Add-VMHardDiskDrive -VMName $Name -Path $VHDPath\$Name.vhdx
    "Added VHD to VM"
    Add-VMDvdDrive -VMName $Name
    "Added extra DVD Drive"
    Set-VMDvdDrive -VMName $Name -ControllerLocation 1 -Path "$UnattendFolder\AutoUnattend.iso"
    "Mounted Unattend ISO"
    Set-VMDvdDrive -VMName $Name -ControllerLocation 0 -Path $IsoFile
    "Mounted OS Install ISO"
    ...
    Set-VMBios $Name -StartupOrder @("IDE","CD","LegacyNetworkAdapter","Floppy")
    #Connect to an existing network switch
    Connect-VMNetworkAdapter -VMName $Name -SwitchName $Config.SwitchName
    start-vm $Name
...
#  New-VM -Name $vm -MemoryStartupBytes 4GB -Generation 1
#  Set-VM $vm -ProcessorCount 4
#  Set-VM $vm -AutomaticCheckpointsEnabled 0
#
#  New-VHD -Path $VHDPath\$vm.vhdx -SizeBytes 64GB -Dynamic
...
#  Set-VMDvdDrive -VMName $vm -ControllerLocation 0 -Path "D:\Source\Windows Server 2022\20348.169.210806-2348.fe_release_svc_refresh_SERVER_EVAL_x64FRE_en-us.iso"
#
#  #First time startup IDE will be ignored because there's nothing there, so it will jump to CD. After install we will no longer boot from CD.
